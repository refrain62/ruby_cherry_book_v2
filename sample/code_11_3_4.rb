...
case {name: 'Alice', age: 20, gender: :female}
    in {name: 'Alice', gender:}
        # in節に:ageを指定していないが、:nameと:genderの条件が部分一致するので全体としてマッチ
        "gender=#{gender}"
end 
#=> "gender=female"
...
#=> syntax error, unxpected symbol literal, expecting label or ** or **arg or string literal(SyntaxError)
...
#=> syntax error, unxpected ',', epecting '}'(SyntaxError)

case {name: 'Alice', age: 20, gender: :femele}
    in {name: 'Alice', **}
...
#=> {:name=>"Alice", :age=>20, :gender=>:femeale}(NoMatchingPatternError)
