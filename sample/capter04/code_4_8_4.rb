...
# 面積の計算結果を格納する配列
areas = []
# ブロックパラメータが1個であえれば、ブロックパラメータの値が配列になる
...
# lengthとwidthには値が渡されるが、fooとbar は nilになる
...
# 3つの値をブロックパラメータに渡そうとするが、2つしかないので３つ目の値は捨てられる
dimensions.each do |length, width|
    p [length, width]
end
#=> [10, 20]
...
# ()を使わない場合は dimensionに配列[10, 20]が代入される
dimension, i = [[10, 20], 0]
dimension #=> [10, 20]
i         #=> 0

# ()を使うと 内側の配列の要素(10と20)を別の変数に代入できる
